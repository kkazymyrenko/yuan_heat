...
def sampleGate(cir):
    """ sample assigned circuit via statevector """
    nqb_loc = cir.num_qubits
    cir.measure_all()
    job_sol = sampler.run([cir])
    res_sol = job_sol.result().quasi_dists[0] # probabilities
    res_ampl = np.zeros(2 ** nqb_loc)   # amplitudes
    for i in range(2 ** nqb_loc):
        res_ampl[i] = np.sqrt(res_sol[i]) if i in res_sol else 0
    return res_ampl
...
dth = np.linspace(-0*np.pi, 2*np.pi, num_para*30)

fff = np.zeros(len(dth))
for ii in range(len(dth)):
    # init_para = np.arange(num_para)/10.
    init_para = dth[ii]*np.ones(num_para)
    fff[ii] = cost_func(init_para)
plt.plot( dth, fff,'o-', linewidth=1.0, label=f'cost for {nqb} qbits')
plt.legend()
plt.show()
print()
exit()
